#!/usr/bin/env node
/*
  Runs the SQL in scripts/bq_proc_events.ddl.sql via the bq CLI.
...
function main() {
  ...
  if (!project) {
    console.error('[bq-ddl] ERROR: No project detected. Run `gcloud config set project <id>` or set GOOGLE_CLOUD_PROJECT.');
    process.exit(2);
  }
  const ddlPath = path.resolve(__dirname, '..', 'scripts', 'bq_proc_events.ddl.sql');
  ...
  if (res.status !== 0) {
    console.error('[bq-ddl] ERROR: bq query failed. Ensure Cloud SDK is installed (`gcloud components install bq`).');
    process.exit(res.status || 1);
  }
  console.log('[bq-ddl] Done.');
}
